...
>    有效
>    https://en.wikipedia.org/wiki/Run-time_type_information(RTTI)
>    
> 　虚函数对于正确的基类指针转换为子类指针是非常重要的。
> 　所以通过这个虚函数表，我们就可以知道该类对象的父类，
> 　在转换的时候就可以用来判断对象有无继承关系。
...
>  
> 
> If the downcast is safe the operator returns the appropriately cast pointer. 
> 
> If the downcast is not safe, the operator returns 0
> 
...
> 
> ```
> ### **3.3 引用并不是指针(References Are Not Pointers )** ###
> ```c
...
>  1 指针 如归null 无法修改内容
>  2 返回值null ，引用定义失败
> 如果一个函数返回值或者dynamic_cast返回值是引用类型。一定抛出异常
> https://www.cnblogs.com/QG-whz/p/4909359.html
> ```

[](https://images2015.cnblogs.com/blog/610439/201510/610439-20151025200700974-1784981192.png)
