 ...
 *   
 */
#define IPADDRESS   "10.112.179.21"
#define PORT        9000
...
int main(int argc,char *argv[]) {
    ...
    if (listenfd <0) {
        printf("socket_bind faild");
        return -1;
    }
    //http://man7.org/linux/man-pages/man2/listen.2.html
    //backlog https://www.jianshu.com/p/30b861cac826
...
static void do_read(int epollfd, int fd, char *buf) {
    ...
    if (nread == -1) {
        ...
        printf("Read:%d message is : %s",fd, buf);
        //work 
        //
...
static void do_write(int epollfd, int fd, char *buf) {
    ...
    if (nwrite == -1) {
        ...
        printf("wirte %d message is : %s",fd,buf);
